...
* **Seguridad:** Managed Identity (Zero Trust), Escaneo IaC (Checkov) y Escaneo de Imágenes (Trivy). [cite: 6, 25, 26, 35]
* **Automatización:** GitFlow con pipelines diferenciados para Infraestructura (IaC) y Aplicación. [cite: 28, 32]
...
* Esto disparará el workflow `Terraform Plan & Apply`. [cite: 33, 37]
* **Nota:** Debes esperar a que este pipeline finalice con éxito antes de intentar desplegar la aplicación, ya que los recursos (ACR, Container App Environment) deben existir previamente. [cite: 43]
...
3.  Haz clic en el enlace de la FQDN generada para ver el JSON con el mensaje de éxito y el secreto recuperado de Key Vault. [cite: 42]
...
### Pipelines de CI/CD
* **`deploy-iac-plan-apply.yml`**: Gestiona el ciclo de vida de Terraform (Init, Plan, Apply). Incluye escaneo de seguridad estático. [cite: 35, 37]
* **`deploy-app.yml`**: Construye la imagen Docker, ejecuta escaneo de vulnerabilidades con **Trivy**, sube la imagen al **ACR** y actualiza la revisión de la Container App. [cite: 34, 35, 36]
...
👉 [PROMPTS.md](./PROMPTS.md) [cite: 22, 47]

---
**Candidato:** DevSecOps Engineer - TechFlow Challenge 2026. [cite: 1, 7]